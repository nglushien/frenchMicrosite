$(document).ready(function(){
    ...
    // Set the minimum height of #micrositeContentColumnMiddle equal to the greater height of the right OR left column.

		var leftCol = $('#micrositeContentColumnLeft').outerHeight(true);
		var rightCol = $('#micrositeContentColumnRight').outerHeight(true);

		if(leftCol > rightCol){
			$('#micrositeContentColumnMiddle').css('min-height', leftCol);
			$('#micrositeContentMainColumn').css('min-height', leftCol);
		} else {
			$('#micrositeContentColumnMiddle').css('min-height', rightCol);
	...
	// Set the position of menu items based on the number of items and the total width of all menu items.
	...
	//Apply the .active class to the correct menu item based on URL pathname and href of the menu item
	...
	// Create your slider below. Don't forget to also move the next/previous buttons.
	...
	// First determine if a next button is necessary. If it's not, gray out the next button. Then determine upon next/previous button clicks whether to gray out previous/next buttons.
	...
	// Move the modified next and previous buttons to their new homes! Have to do it here or you lose the style "gray out" style change determined above.
		...
		$('.slider2 .bx-prev').appendTo('.slider2 > .previousButton');
	
	// Calculate the height of the individual paragraph(+ span) within the li and then center that text in the li. This works for a single paragraph(+ span).
	
		$('.bxslider li').each(function(){
			var pHeight = $('p', this).height();
			var sHeight = $('span', this).height();
			var totalHeight = pHeight + sHeight;
			pTop = 40 - (totalHeight/2);
			$('p',this).css('padding-top', pTop);
		});
